function spoilerToggle(event) {
  ...
  if($(sliderID).css("display") == "none") {
    $(this).text("[-]");
	 $(this).attr("title", "Tutup Spoiler")			
  }
  else {
	 $(this).text("[+]");
	 $(this).attr("title", "Lihat Spoiler")
  }
...
function addInlineSpoiler(i) {
  ...
  $("<span class='inline-spoiler-toggle' title='Lihat Spoiler' slider-id='I" + i + "'>[+]</span>").insertBefore(this);
}
function addBlockSpoiler(i) {
  $(this).attr("id", "spoiler-sliderB" + i);
  $("<div class='block-spoiler'>spoiler <span class='block-spoiler-toggle' title='Lihat Spoiler' slider-id='B" + i + "'>[+]</span></div>").insertBefore(this);
}
